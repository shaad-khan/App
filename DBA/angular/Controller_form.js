...
$interval(callApi, 1000);
  ...
  function callApi(){
    $scope.date = new Date();
    $http.get("https://apps.continuserve.com/DBA/webservice/ticket_data.php?ID="+$scope.ID).then(function(response){
     	//$scope.load2='false';
...
$http.get("https://apps.continuserve.com/DBA/webservice/updateinfo.php?ID="+$scope.ID).then(function(response){
     	//$scope.load2='false';
...
{
  console.log($url);
return $http.get("https://apps.continuserve.com/DBA/webservice/service.php?type="+$url);
};
