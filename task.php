...
<div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" ng-controller="addtask">
  ...
  <option ng-repeat="tt in tts | orderBy:'Task'" value="{{tt.Task}}">{{tt.Task}}</option>
        ...
        --><textarea class="form-control"   rows="3" cols="110" name="messages" placeholder="messages" ng-model="myTextarea"  ng-blur="nupdate()" ng-show="tasktype!='Email Review and Self learning'"></textarea>
 <!-- message{{amessage}}-->
     <!-- <input type="text" ng-model='xx' ng-if="tasktype!='Email Review and Self learning'" value="{{myTextarea}}" />
       -->  </div>
